...
def render_modify_pn():
    ...
    if st.session_state.pn_data:
        ...
        if selected_pn:
            with st.expander(f"PN: {selected_pn} ({PN_MODEL_MAPPING.get(selected_pn, 'Inconnu')})", expanded=True):
                st.markdown(f"**Fichier associé** : {st.session_state.pn_file_name.get(selected_pn, 'N/A')} | **Modèle** : {PN_MODEL_MAPPING.get(selected_pn, 'Inconnu')}")
                ...
                with st.form(key=f"modify_pn_form_{selected_pn}"):
                    ...
                    with col_update:
                        if st.form_submit_button(f"Mettre à jour {selected_pn}"):
                            ...
                            st.success(f"PN {selected_pn} mis à jour avec succès !")
                            st.rerun()
                    with col_delete:
